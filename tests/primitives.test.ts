...
test("lx.unknown()", () => {
	...
	expect(result.json).toEqual({ type: "unknown" });
});

test("lx.bytes()", () => {
	const result = lx.bytes();
	expect(result.json).toEqual({ type: "bytes" });
});

test("lx.bytes() with minLength", () => {
	const result = lx.bytes({ minLength: 1 });
	expect(result.json).toEqual({ type: "bytes", minLength: 1 });
});

test("lx.bytes() with maxLength", () => {
	const result = lx.bytes({ maxLength: 1024 });
	expect(result.json).toEqual({ type: "bytes", maxLength: 1024 });
});

test("lx.bytes() with minLength and maxLength", () => {
	const result = lx.bytes({ minLength: 1, maxLength: 1024 });
	expect(result.json).toEqual({ type: "bytes", minLength: 1, maxLength: 1024 });
});
...
test("lx.blob()", () => {
	...
	expect(result.json).toEqual({ type: "blob" });
});
...
test("lx.blob() with maxSize", () => {
	...
	expect(result.json).toEqual({ type: "blob", maxSize: 1000000 });
});
...
test("lx.array() with string items", () => {
	...
	expect(result.json).toEqual({ type: "array", items: { type: "string" } });
});

test("lx.array() with integer items", () => {
	const result = lx.array(lx.integer());
	expect(result.json).toEqual({ type: "array", items: { type: "integer" } });
});
...
test("lx.query() basic", () => {
	...
	expect(result.json).toEqual({ type: "query" });
});
...
test("lx.procedure() basic", () => {
	...
	expect(result.json).toEqual({ type: "procedure" });
});
...
test("lx.subscription() basic", () => {
	...
	expect(result.json).toEqual({ type: "subscription" });
});
